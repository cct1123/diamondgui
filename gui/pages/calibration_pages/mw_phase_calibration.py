...
from gui.task_config import JM, TASK_MW_PHASE_CALIBRATION
...
IDLE_INTERVAL = 500
ID = TASK_MW_PHASE_CALIBRATION.get_uiid()
...
layout_MWPhaseTune = dbc.Col(
    [
        dbc.Card(
            [
                dbc.CardHeader(
                    [html.H4("MW Phase Calibration", className="mt-0 mb-0")]
                ),
                dbc.CardBody(
                    [
                        dbc.Row(
...
@callback(
    ...
    Input(ID + "-input-ODMR_freq", "value"),
...
def update_params(
    ...
    paramsdict = dict(
        mw_phasevolt_start=mw_phasevolt_start,
        mw_phasevolt_stop=mw_phasevolt_stop,
        mw_phasevolt_step=mw_phasevolt_step,
    ...
    TASK_MW_PHASE_CALIBRATION.set_paraset(**paramsdict)
    TASK_MW_PHASE_CALIBRATION.set_priority(int(priority))
    TASK_MW_PHASE_CALIBRATION.set_stoptime(stoptime)
    return {}
...
def check_run_stop_exp(_r, _p, _s):
    ...
    if ctx.triggered_id == ID + "-button-start":
        ...
        if TASK_MW_PHASE_CALIBRATION.state == "run":
...
def _run_exp():
    JM.start()
    JM.submit(TASK_MW_PHASE_CALIBRATION)
    # return False, True, True, DATA_INTERVAL
    return DATA_INTERVAL, STATE_INTERVAL


def _pause_exp():
    TASK_MW_PHASE_CALIBRATION.pause()
...
def _stop_exp():
    JM.start()
    JM.remove(TASK_MW_PHASE_CALIBRATION)
...
def update_store_state(_):
    return TASK_MW_PHASE_CALIBRATION.stateset
...
def update_store_parameters_data(_):
    return TASK_MW_PHASE_CALIBRATION.paraset, TASK_MW_PHASE_CALIBRATION.dataset
...
@callback(
    ...
    Output(ID + "-input-ODMR_freq", "disabled"),
