...
layout_para = dbc.Col([
    ...
    dbc.Row([
        dbc.Col([
            ...
            # ),
            dbc.Row([UnitedInput("Min Volt", -10.0, 10.0, 1E-7, -10.0, "V", id=ID+"input-min volt")]),
            dbc.Row([UnitedInput("Max Volt", -10.0, 10.0, 1E-7, -10.0, "V", id=ID+"input-min volt")]),
            dbc.Row([NumericInput("Number Average", 1, 8333, 1, 200, id=ID+"input-number average")]),
            dbc.Row([UnitedInput("Sampling Rate", 1.0, 500E3, 0.1, 100E3, "Hz", id=ID+"input-sampling rate")]),
            dbc.Row([NumericInput("Refresh Rate", 1.0, 60.0, 0.1, 30.0, id=ID+"input-refresh rate")]),
            dbc.Row([UnitedInput("History Duration", 0.0, 3600, 1E-6, 5.0, "s", id=ID+"input-history_duration")]),
        ]),
    ]),
])
