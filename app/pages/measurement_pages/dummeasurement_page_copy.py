...
dumdumodmr_copy = DummyODMR(name="copy")
jm = JobManager()
def release_lock():
    return jm.stop()
...
layout_exppara = dbc.Row([
    dbc.Col([
        dbc.Row([
            NumericInput(
                "B Field", min=-1000.0, max=1000.0, step=1E-3, value=5.0, unit="G",
                id=ID + "-input-bfield", persistence_type="local"
            ),
        ]),
        dbc.Row([
            NumericInput(
                "MW Power", min=0.0, max=1000.0, step=1E-3, value=5.0, unit="%",
                id=ID + "-input-mwpower", persistence_type="local"
            ),
        ]),
        dbc.Row([
            NumericInput(
                "Freq Begin", min=0.0, max=10.0, step="any", value=2.0, unit="GHz",
                id=ID + "-input-freq_begin", persistence_type="local"
            ),
        ]),
        dbc.Row([
            NumericInput(
                "Freq End", min=0.0, max=10.0, step="any", value=4.0, unit="GHz",
                id=ID + "-input-freq_end", persistence_type="local"
            ),
        ]),
        dbc.Row([
            NumericInput(
                "Freq Step", min=0.0, max=1.0, step="any", value=0.01, unit="GHz",
...
layout_dummyODMR_copy = html.Div([
    dbc.Row([
...
def update_params(mwpower, bfield, freq_begin, freq_end, freq_step, stoptime):
    ...
    dumdumodmr_copy.set_paraset(**paramsdict)
    dumdumodmr_copy.set_stoptime(stoptime)
    return {}
# ---------------------------------------------------------------------------------------------
...
def update_state_parameters_data(_):
    storedata = dict()
    storedata = dict(stateset=dumdumodmr_copy.stateset, paraset=dumdumodmr_copy.paraset, dataset=dumdumodmr_copy.dataset)
...
if __name__ == "__main__":
    ...
    app = dash.Dash(
        ...
        external_scripts=[])
    app.layout = layout_dummyODMR_copy
    app.run_server(
        # host="0.0.0.0", 
        debug=DEBUG, 
        port=GUI_PORT)